...
## What This Is
A shared memory service with knowledge graph for all Claude instances across the VPN (<vpn-host>/24).
...
- **Deployed via Coolify** on bimavo (<vpn-host>)
- **Dashboard** at nobrainr.example.internal (VPN-only via Traefik ipAllowList)
...
nobrainr import-claude /root/.claude --machine bimavo
```

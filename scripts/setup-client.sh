...
#
# Expects: machine has Claude Code installed, connectivity to <vpn-host>:8420

set -euo pipefail

NOBRAINR_URL="http://<vpn-host>:8420"
...
if [[ -f "$MCP_FILE" ]]; then
    ...
    jq '.mcpServers.nobrainr = {"type": "sse", "url": "http://<vpn-host>:8420/sse"}' \

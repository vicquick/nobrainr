...
#
# Expects: machine has Claude Code installed, connectivity to 10.0.0.2:8420

set -euo pipefail

NOBRAINR_URL="http://10.0.0.2:8420"
MYSERVER_IP="10.0.0.2"
...
if [[ -f "$MCP_FILE" ]]; then
    jq '.mcpServers.nobrainr = {"type": "sse", "url": "http://10.0.0.2:8420/sse"}' \
...
# 5. Copy nobrainr scripts from myserver (single source of truth)
scp -q "root@${MYSERVER_IP}:~/.claude/scripts/nobrainr-query.py" "$SCRIPTS_DIR/nobrainr-query.py"
chmod +x "$SCRIPTS_DIR/nobrainr-query.py"
echo "Deployed: $SCRIPTS_DIR/nobrainr-query.py"

scp -q "root@${MYSERVER_IP}:~/.claude/scripts/nobrainr-recall.sh" "$SCRIPTS_DIR/nobrainr-recall.sh"
